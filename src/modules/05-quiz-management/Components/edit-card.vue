<template>
    <div>
                            ...
                            <v-card>
                                ...
                                <v-container grid-list-md>
                                    <v-flex>
                                        ...
                                        <v-select v-model="category" :items="categories" item-text="category" item-value="value" label="Select Category" :rules="[rules.required]" return-object single-line></v-select>
                                    </v-flex>
                                </v-container>
        ...
        <v-dialog width=350 v-model="confirm">
            <v-card>
                <v-card-title>
                    <v-layout justify-center align-center>Confirm Quiz Update</v-layout>
                </v-card-title>
...
<script>
    ...
    methods: {
            ...
            newImg: function(){//As the image is locally defined as an object, but defined in firebase by its url, it will need to be matched to the iobject for it to load into the select component
                for(var i=0;i<this.items.length;i++) {//Searches the local list
